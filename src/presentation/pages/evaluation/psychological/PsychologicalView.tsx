...
import { PsychologicalContainer } from '../../../../di/evaluation/PsychologicalContainer';
...
const PsychologicalView: React.FC = memo(() => {
  ...
  const PsychologicalViewModel = PsychologicalContainer.resolve('psychologicalViewModel');
  ...
  const printEvaluation = async (id:number) => {
    setLoading(true)
    try {
      await PrintViewModel.printEvaPsychological(id);
    } catch (error) {}
    setLoading(false)
  };
