...
import { exportMedicalReportPdf as exportToPDF } from '../../../utils/reports/exportMedicalReportPdf';
import { exportMedicalReportExcel as exportToExcel } from '../../../utils/reports/exportMedicalReportExcel';
...
const ReportMedicalView: React.FC = memo(() => {
  ...
  const handleReportPdf = useCallback(async () => {
    ...
    // Preparar parametros especificos para el reporte medico
    const sucursalSel = listSucursales.find(s => String(s.id_sucursal) === String(getValues('id_sucursal')));
    const userSel = listUsers.find(u => String(u.id_usuario) === String(getValues('id_usuario')));

    setLoading(true);
    await exportToPDF({
      data: dataWithIndex,
      titleDoc: `Reporte evaluacion medica ${fecha}`,
      centroMedico: sucursalSel ? sucursalSel.nombre : '',
      nombreMedico: userSel ? userSel.nombre + (userSel.ap_paterno ? ' ' + userSel.ap_paterno : '') : '',
    })
    setLoading(false);
  }, [listReport])
  ...
  const handleReportExcel = useCallback(async () => {
    ...
    const sucursalSel = listSucursales.find(s => String(s.id_sucursal) === String(getValues('id_sucursal')));
    const userSel = listUsers.find(u => String(u.id_usuario) === String(getValues('id_usuario'))) as any;
        ...
        <Grid key={'buttom-header'} container size={{ xs: 5.5, sm: 5, md: 4 }} justifyContent="end" alignItems="end" alignSelf={'center'}>
          {buttons.filter((boton: Button) => boton.tipo === 'header')
            .map((button: any, index: number) => {
              ...
              return (
                <Tooltip key={`${button.title || 'btn'}-${index}`} title={button.tooltip}>
