...
interface UserFormProps {
  open: boolean;
  onClose: () => void;
  updateAccesProfile: (idAcces:number,idUser:string,estado:number,buttons:string[]) => void;
  accesProfile?: AccesUser | null;
  profile?: Perfil | null;
  buttons:Button[];
  buttonsActive:Button[];
}
const ModalAccesProfile: React.FC<UserFormProps> = ({ open, onClose,updateAccesProfile,buttons,accesProfile,profile,buttonsActive}) => {
  ...
  useEffect(() => {
    if (accesProfile) {
      setStateAcces(accesProfile.acceso==='1');
      if (Array.isArray(accesProfile.botones)) {
          setSelectedButtons(accesProfile.botones);
      }
      if (Array.isArray(accesProfile.id_botones)) {
        setAccesButtons(accesProfile.id_botones);
    }
    } else { 
        setSelectedButtons([]);
    }
  ...
  const handleUpdate = useCallback(async () => {
    await updateAccesProfile(Number(accesProfile?.id_menu_acceso),String(profile?.id),stateAcces?1:0,selectedButtons);
  }, [accesProfile,profile,selectedButtons,stateAcces]);
        ...
        <form >
          <ScrollableBox>
            <Grid container spacing={3}>
              ...
              <SwitchSimple
                  id={'acces'}
                  label={'Acceso'}
                  icon={React.createElement(MUIcons[accesProfile?.icono as keyof typeof MUIcons] || MUIcons.Label)}
          ...
          <ContainerButtons>
            ...
            .map((button: any,index:number) => {
              if (accesProfile?.id_menu_acceso && button?.tipo === 'table') {
                return (
                  <ActionButton
...
export default ModalAccesProfile;