...
import { MaterialReactTable, MRT_DensityState, useMaterialReactTable, type MRT_ColumnDef } from 'material-react-table';
...
const TableAccesUser = (Props: any) => {
  const { listAccesUser, handleOpenModalAccesUser } = Props;
  const firstMenuAcces: AccesUser | null = listAccesUser ? listAccesUser[0] ?? null : null;

  const [density, setDensity] = useState<MRT_DensityState>('compact');
  const [isFullScreen, setIsFullScreen] = useState(false);
  const columns = useMemo<MRT_ColumnDef<AccesUser>[]>(
    () => [
      {
        accessorKey: 'nombre',
        header: 'Nombre',
  ...
  const table = useMaterialReactTable({
    ...
    muiTableContainerProps: {
      sx: {
        marginTop:0,
        height: {xs:isFullScreen?'90%':'64vh',sm:isFullScreen?'90%':'64vh',md:isFullScreen?'100%':'64vh'},
      },
    },
        ...
        Cell: ({ row, table }) => (
          <div
            style={{
              ...
              alignItems: 'center',
              //width: '100%',
              //paddingLeft: `${row.depth * 20}px`, // Indentación
            }}
          >
          {/*row.depth>0 &&
                Array.from({ length: row.depth }).map((_, i) => {
                const lineColor = adjustHslLuminosity(hexToHsl(palette.secondary.dark), i* 20); // Color del padre de esta línea (i.e. el color de la profundidad `i`)
                return (
                  <Divider orientation="vertical" sx={{marginLeft: `${(1) * 20}px`,marginRight:'4px',backgroundColor:lineColor,width: '4px',height:density === 'compact'?'45px':density==='comfortable'?'60px':'75px'}} flexItem />
                );
              })*/}
      ...
      return {
        onClick: (event:any) => {
          if (!event.target.closest('.MuiButtonBase-root')) {
            handleOpenModalAccesUser(row.original);
          }
        },
        sx: {
          cursor: 'pointer',
