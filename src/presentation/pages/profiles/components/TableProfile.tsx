...
const TableProfile = (Props: any) => {
  const { listProfile, handleOpenModalProfile } = Props;
  
  const firstProfile: Perfil|null = listProfile[0]??null;
  const columns = useMemo(
    () =>
      firstProfile
        ? Object.keys(firstProfile)
            .filter(key => {return key !== 'foto';})
            .map(key => ({
              accessorKey: key as keyof Perfil,
  ...
  const table = useMaterialReactTable({
    ...
    muiTableBodyRowProps: ({ row }) => {
      ...
      return {
        onClick: (event) => {
          handleOpenModalProfile(row.original);
        },
        sx: {
          cursor: 'pointer',
