import React, { useState, useMemo } from 'react';
import { MaterialReactTable, useMaterialReactTable } from 'material-react-table';
import { Box,createTheme, ThemeProvider } from '@mui/material';
import { MRT_Localization_ES } from 'material-react-table/locales/es';
import { PaymentMethod } from '../../../../../../domain/models/PaymentMethodModel';

const TablePaymentMethod = (Props: any) => {
  const { listPaymentMethods, handleOpenModalPaymentMethod } = Props;
  const firstPaymentMethod: PaymentMethod|null = listPaymentMethods[0]??null;
  const columns = useMemo(
    () =>
      firstPaymentMethod
        ? Object.keys(firstPaymentMethod)
            .map(key => ({
              accessorKey: key as keyof PaymentMethod,
              header: key.replace(/_/g, ' ').replace(/^\w/, c => c.toUpperCase()),
            }))
        : [],
    [firstPaymentMethod], 
  );

  const table = useMaterialReactTable({
    columns,
    data: listPaymentMethods,
    enableRowSelection: false,
    enableRowActions: false,
    localization: MRT_Localization_ES,
    renderRowActionMenuItems: ({ row }) => [
    ],
    muiTableBodyCellProps: ({ column,row}) => {
      return {
        children: column.id === 'estado'? row.original.estado === '1'? 'Activo': 'Eliminado': undefined,
        sx: {
          fontSize:column.id === 'estado'?'16px':'auto',
          color: column.id === 'estado' ? row.original.estado === '1' ? 'green' : 'red' : 'inherit',
        },
      };
    },
    muiTableBodyRowProps: ({ row }) => {
      const style = {};
      return {
        onClick: (event) => {
          handleOpenModalPaymentMethod(row.original);
        },
        sx: {
          cursor: 'pointer',
          backgroundColor:row.original.estado == 0?'#f7d9d4': (row.index % 2 === 0 ? '#f9f9f9' : '#ffffff'),
          '&:hover': {
            backgroundColor: '#f0f0f0',
          },
        },
      };
    },
  });

  const customTheme = createTheme({
    palette: {
      primary: {
        main: '#1976d2', 
      },
      secondary: {
        main: '#dc004e', 
      },
    },
  });

  return (
    <Box>
      {listPaymentMethods!.length > 0 && (
        <ThemeProvider theme={customTheme}>
          <MaterialReactTable table={table} />
        </ThemeProvider>
      )}
    </Box>
  );
};

export default TablePaymentMethod;