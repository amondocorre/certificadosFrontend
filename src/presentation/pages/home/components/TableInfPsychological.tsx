...
import { InfEvaluantionPsychological } from '../../../../domain/models/DashboardModel';
const colors:string[] = ['#8cf892ff','#f5f77fff','#e67364ff']
const TableInfPsychological = (Props: any) => {
  const { handleOpen,listEvaPsychological,id_sucursal} = Props;
  const [data, setData] = useState<InfEvaluantionPsychological[]>([]);
  ...
  const columns = useMemo<MRT_ColumnDef<InfEvaluantionPsychological>[]>(
    () => [
      { accessorKey: 'id_inf_evaluacion_psicologica', header: 'N°',size:10},
      { accessorKey: 'nombre_completo', header: 'Nombre Completo',size:100},
      { accessorKey: 'fecha_registro',header:'Fecha Registro', size:20,
        Cell: ({ row }) => (
          <Box>
            {String(formatDateDMY(row?.original.fecha_registro))}
          </Box>
        ),},
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: () => (
      <Box sx={{width:'100%'}}>
        <StyledTitle sx={{fontSize: '25px',}}>{'Informe de Evaluaciones Psicologicas'}</StyledTitle>
      </Box>
    ),
    ...
    muiTableBodyRowProps: ({ row }) => {
      ...
      return {
        onClick: () => {
          handleOpen(Number(row.original.id_inf_evaluacion_psicologica));
        },
        sx: {
...
export default TableInfPsychological;
